...
struct Surface *check_ledge_grab(struct MarioState *m, struct Surface *prevWall, struct Surface *wall, Vec3f intendedPos, Vec3f nextPos, Vec3f ledgePos, struct Surface **ledgeFloor) {
    ...
    if (ledgeFloor == NULL
        ...
        || (configSteepSlopeJumps && (*ledgeFloor)->normal.y < COS25) // H64 TODO: check if floor is actually slippery
#endif
    ) {
        return NULL;
    }
...
s32 perform_air_quarter_step(struct MarioState *m, Vec3f intendedPos, u32 stepArg) {
    ...
    if (nextPos[1] + 160.0f > ceilHeight) {
        if (m->vel[1] >= 0.0f) {
            ...
            int c1 = !(m->prevAction & ACT_FLAG_HANGING) && ceil != NULL && ceil->type == SURFACE_HANGABLE;
            int c2 = (stepArg & AIR_STEP_CHECK_HANG) && ceil != NULL && ceil->type == SURFACE_HANGABLE;

            int cond = configSteepSlopeJumps ? c1 : c2;
            if (cond) {
